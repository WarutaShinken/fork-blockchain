...
ROOT = pathlib.Path(importlib.import_module("fork").__file__).absolute().parent.parent


def solve_name_collision_problem(analysis):
    """
    There is a collision between the `fork` file name (which is the executable)
    and the `fork` directory, which contains non-code resources like `english.txt`.
    We move all the resources in the zipped area so there is no
    need to create the `fork` directory, since the names collide.
...
version_data = copy_metadata(get_distribution("fork-blockchain"))[0]
...
# TODO: collapse all these entry points into one `fork_exec` entrypoint that accepts the server as a parameter

entry_points = ["fork.cmds.fork"] + [f"fork.server.start_{s}" for s in SERVERS]
...
if THIS_IS_WINDOWS:
    entry_points.extend(["aiohttp", "fork.util.bip39"])

if THIS_IS_WINDOWS:
    fork_mod = importlib.import_module("fork")
    dll_paths = ROOT / "*.dll"
...
datas.append((f"{ROOT}/fork/util/english.txt", "fork/util"))
datas.append((f"{ROOT}/fork/util/initial-config.yaml", "fork/util"))
datas.append((f"{ROOT}/fork/wallet/puzzles/*.hex", "fork/wallet/puzzles"))
datas.append((f"{ROOT}/fork/ssl/*", "fork/ssl"))
...
add_binary("fork", f"{ROOT}/fork/cmds/fork.py", COLLECT_ARGS)
add_binary("daemon", f"{ROOT}/fork/daemon/server.py", COLLECT_ARGS)

for server in SERVERS:
    add_binary(f"start_{server}", f"{ROOT}/fork/server/start_{server}.py", COLLECT_ARGS)
