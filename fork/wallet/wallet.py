...
from fork.consensus.cost_calculator import calculate_cost_of_program, NPCResult
...
from fork.wallet.puzzles.puzzle_utils import (
...
from fork.wallet.secret_key_store import SecretKeyStore
from fork.wallet.sign_coin_solutions import sign_coin_solutions
...
class Wallet:
    ...
    async def create_spend_bundle_relative_fork(self, fork_amount: int, exclude: List[Coin]) -> SpendBundle:
        list_of_solutions = []
        utxos = None
        ...
        if fork_amount < 0:
            utxos = await self.select_coins(abs(fork_amount), exclude)
        else:
        ...
        fork_amount = spend_value + fork_amount
        ...
        for coin in utxos:
            ...
            if output_created is None:
                ...
                primaries = [{"puzzlehash": newpuzhash, "amount": fork_amount}]
