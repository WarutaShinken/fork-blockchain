...
import { AlertDialog } from '@fork/core';
import {
  Typography,
  Button,
  Box,
...
import { fork_to_mojo } from '../../../util/fork';
...
export const CreateRLAdminWallet = () => {
  ...
  function create() {
    ...
    if (
      forkper_input.value === '' ||
      Number(forkper_input.value) === 0 ||
      !Number(forkper_input.value) ||
      isNaN(Number(forkper_input.value))
    ) {
      dispatch(
        openDialog(
          <AlertDialog>
    ...
    const forkper = fork_to_mojo(forkper_input.value);
    const forkper_value = Number.parseInt(Number(forkper));
    const userpubkey = userpubkey_input.value;
    const amount = fork_to_mojo(amount_input.value);
    const amount_value = Number.parseInt(Number(amount));
    // var fee = fork_to_mojo(fee_input.value);
