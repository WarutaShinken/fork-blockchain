...
function BalanceCard(props: BalanceCardProps) {
  ...
  const balance_ptotal_fork = mojo_to_colouredcoin_string(balance_ptotal);
  const balance_pending_fork = mojo_to_colouredcoin_string(balance_pending);
  const balance_change_fork = mojo_to_colouredcoin_string(balance_change);
  const acc_content =
    balancebox_1 +
    balancebox_2 +
    balancebox_ptotal +
...
function SendCard(props: SendCardProps) {
  ...
  function send() {
    ...
    if (address.includes('fork_addr') || address.includes('colour_desc')) {
      dispatch(
        openDialog(
          <AlertDialog>
