...
class WalletNode:
    ...
    async def complete_blocks(self, header_blocks: List[HeaderBlock], peer: WSForkConnection):
    ...
    async def _sync(self) -> None:
        ...
        peers: List[WSForkConnection] = self.server.get_full_node_connections()
        if len(peers) == 0:
    ...
    async def get_additions(
        self, peer: WSForkConnection, block_i, additions: Optional[List[bytes32]], get_all_additions: bool = False
    ) -> Optional[List[Coin]]:
    ...
    async def get_removals(
        self, peer: WSForkConnection, block_i, additions, removals, request_all_removals=False
    ) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None
