...
export default function Block() {
  ...
  const poolReward = mojo_to_fork(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojo_to_fork(
    calculateBaseFarmerReward(blockRecord.height),
  );

  const forkFees = blockRecord.fees
    ? mojo_to_fork(BigInt(blockRecord.fees))
    : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.forkexplorer.com/blockchain/puzzlehash/${blockRecord.farmer_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.forkexplorer.com/blockchain/puzzlehash/${blockRecord.pool_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: forkFees ? `${forkFees} ${currencyCode}` : '',
      tooltip: (
        <Trans>
