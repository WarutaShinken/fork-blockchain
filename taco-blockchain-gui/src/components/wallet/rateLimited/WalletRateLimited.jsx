...
import { AlertDialog, Card, Flex } from '@fork/core';
import {
  send_transaction,
...
const IncompleteCard = (props) => {
  ...
  function submit() {
    ...
    const hexfkeck = /[\da-f]+$/gi;

    if (!hexfkeck.test(ip_val) || ip_val.value === '') {
    ...
    const forkper_input = ip_parsed.limit;
    ...
    const forkper_value = Number.parseInt(Number(forkper_input));
        ...
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>
                  Spending Limit (fork per interval):{' '}
                  {mojo_to_fork_string(limit)}
                </Trans>
              </Typography>
            </Box>
          </Box>
...
const SendCard = (props) => {
  ...
  function send() {
    ...
    const fee = fork_to_mojo(fee_input.value);
