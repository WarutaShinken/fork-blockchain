...
from fork.daemon.server import WebSocketServer, create_server_for_daemon, daemon_launch_lock_path, singleton
from fork.full_node.full_node_api import FullNodeAPI
from fork.server.start_farmer import service_kwargs_for_farmer
from fork.server.start_full_node import service_kwargs_for_full_node
from fork.server.start_harvester import service_kwargs_for_harvester
from fork.server.start_introducer import service_kwargs_for_introducer
from fork.server.start_service import Service
from fork.server.start_timelord import service_kwargs_for_timelord
from fork.server.start_wallet import service_kwargs_for_wallet
from fork.simulator.start_simulator import service_kwargs_for_full_node_simulator
from fork.timelord.timelord_launcher import kill_processes, spawn_process
...
async def setup_farmer(
    ...
    config["xfk_target_address"] = encode_puzzle_hash(b_tools.farmer_ph, "xfk")
    config["pool_public_keys"] = [bytes(pk).hex() for pk in b_tools.pool_pubkeys]
    config["port"] = port
    config_pool["xfk_target_address"] = encode_puzzle_hash(b_tools.pool_ph, "xfk")
